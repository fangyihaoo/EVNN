...
from utils import WillmoreLoss
...
def train(**kwargs):
    ...
    energy = []
    previous = []
    init_path = osp.join(osp.dirname(osp.dirname(osp.realpath(__file__))), 'checkpoints', 'Willmore', config.pretrain)
    ...
    for epoch in range(int(config.max_epoch)):
        ...
        model.save(f'Willmore/twoEllipseWillmore{epoch+1}.pt')
        modelold.load_state_dict(model.state_dict())
        
    log_path = osp.join(osp.dirname(osp.dirname(osp.realpath(__file__))), 'data', 'Willmore',  'twoEllipseEnergy.pt')
